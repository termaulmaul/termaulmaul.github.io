      ...
      <div className="flex flex-col md:flex-row gap-12 items-start justify-between">
        ...
        <div className="w-full md:w-2/3 flex flex-col gap-8">
          {Object.entries(profile.techStack).map(([category, tools]) => (
            <div key={category} className="card-glass-light p-8">
              <p className="mb-6 text-[14px] text-[#52525b] tracking-widest uppercase font-sans font-bold flex items-center gap-2">
                {category}
              </p>
              <div className="flex flex-wrap gap-2">
                {tools.map(tool => (
                  <span key={tool} className="badge-light">
                    {tool}
                  </span>
                ))}
              </div>

    ...
    <section id="about" className="py-24 px-6 max-w-[1100px] mx-auto scroll-mt-12">
      <div className="flex flex-col md:flex-row gap-12 items-start justify-between">
        ...
        <div className="w-full md:w-2/3 flex flex-col gap-8">
          <p className="text-[20px] md:text-[28px] font-sans font-medium text-[#ffffff] leading-tight tracking-tight">
          ...
          <div className="grid grid-cols-2 gap-4 mt-8">
            {profile.about.map((item) => (
              <div key={item.label} className="flex flex-col gap-1">
                ...
                <span className="text-[#ffffff] text-[16px] font-medium font-sans">
                  {item.value}
                </span>
              </div>
            ))}
